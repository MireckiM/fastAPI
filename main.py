...
@app.post("/api/token/")
async def generate_token(form_data: OAuth2PasswordRequestForm = _fastapi.Depends()):
    user = await _services.authenticate_user(username=form_data.username, password=form_data.password)
